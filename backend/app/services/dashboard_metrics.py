...
def _attention_items(*, jobs, connectors, sources, expiring, health, coverage, backup, now, high_iocs_open) -> list[dict[str, Any]]:
...
def render_executive_brief(summary: dict[str, Any]) -> str:
    ...
    def trend(metric: dict[str, Any]) -> str:
        ...
        return f'{metric["value"]} {arrow} {abs(delta)}{pct} vs previous period'
    ...
    lines = [
        "# SOC Executive Brief", "",
        f'**Period:** last {summary["period"]["days"]} days (until {summary["generated_at"][:10]})',
        f'**Security posture:** {posture["score"]}/100 (grade {posture["grade"]})', "",
        "## Headline KPIs", "",
        "| KPI | Value |", "|---|---|",
        f'| Active investigations | {kpis["active_investigations"]} ({kpis["red_investigations"]} red) |',
        f'| New IoCs | {trend(kpis["iocs_new"])} |',
        f'| High/critical IoCs active | {kpis["high_severity_iocs_open"]} |',
        f'| Intelligence items | {trend(kpis["intel_items"])} |',
        f'| Documents processed | {trend(kpis["documents_processed"])} |',
        f'| Reports delivered | {trend(kpis["reports_delivered"])} |',
        f'| Automation success rate | {pct(kpis["job_success_rate"])} |',
        f'| Integration readiness | {pct(kpis["integration_readiness"])} |',
        f'| ATT&CK tactic coverage | {pct(kpis["sensor_coverage"])} |',
        f'| Engaged users | {kpis["users"]["engaged"]} of {kpis["users"]["active"]} active |',
        "", "## Posture components", "",
    ...
    if summary["attention"]:
        ...
        lines.append("- Nothing requires executive attention in this period.")
    lines.extend(["", "## Investigation portfolio", ""])
    if summary["portfolio"]["projects"]:
        lines.extend(["| Project | Customer | Health | Progress | Risks |", "|---|---|---|---|---|"])
        lines.extend(
    ...
    if summary["coverage"]["gaps"]:
        lines.extend(["", "## Detection gaps", "", ", ".join(summary["coverage"]["gaps"])])
    lines.extend(["", f'_Generated by SOC Workbench at {summary["generated_at"]}_'])
    return "\n".join(lines)
