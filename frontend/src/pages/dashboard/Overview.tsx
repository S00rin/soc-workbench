...
import { useLiveData } from "./shared";
...
const QUICK = [
  ...
  { label: "Executive view", icon: "EX", to: "/?view=executive" },
  { label: "Technical view", icon: "TE", to: "/?view=technical" },
];

export default function Overview({ refreshMs }: { refreshMs: number }) {
  const access = useAccess();
  const nav = useNavigate();
  const { data, err } = useLiveData("/api/dashboard", refreshMs);
