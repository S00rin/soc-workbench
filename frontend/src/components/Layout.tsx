...
export default function Layout({ children, onLogout }: { children: ReactNode; onLogout: () => void }) {
  ...
  const title = TITLES[loc.pathname] || "SOC Workbench";

  return (
    <div className="app">
      <aside className={`sidebar ${open ? "open" : ""}`} onClick={() => setOpen(false)}>
        <div className="brand">
          <span className="logo">◆</span> SOC Workbench
        </div>
        {NAV.map((g) => (
          <div key={g.group}>
      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="row">
            <span className="dim" style={{ fontSize: 12.5 }}>admin</span>
            <button className="btn-sm btn-ghost" onClick={onLogout}>Sign out</button>
          </div>
        </header>
